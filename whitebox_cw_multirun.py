"""
whitebox_cw_multirun.py
Runner C&W seul — extrait de whitebox_multirun.py.

Note : cw_mlp et cw_xgb font un warm-start via un mini-PGD interne
(iters=20, restarts=3) — c'est normal et voulu, ça reste dans whitebox.py,
pas besoin d'y toucher ici.

Usage :
  python whitebox_cw_multirun.py --dataset swat    --eps 0.1
  python whitebox_cw_multirun.py --dataset swat    --eps 0.3
  python whitebox_cw_multirun.py --dataset batadal --eps 0.1
  python whitebox_cw_multirun.py --dataset batadal --eps 0.3

Sorties :
  ~/<dataset>/results/whitebox_cw_<dataset>_eps<eps>.csv
  ~/<dataset>/results/whitebox_cw_<dataset>_eps<eps>.json
  ~/<dataset>/results/whitebox_cw_<dataset>_eps<eps>_tmp.csv  (checkpoint)
...
def run():
    ...
    csv_path = RESULTS_DIR / f"whitebox_cw_{TAG}.csv"
    ...
    json_path = RESULTS_DIR / f"whitebox_cw_{TAG}.json"
