...
FIX (par rapport à la version d'origine) : les fichiers de sortie
(_tmp.csv, .csv, .json) dépendent maintenant de --only. Avant, les 3
familles écrivaient toujours dans le même blackbox_multirun_<tag>_tmp.csv
— si tu lançais les 3 familles en parallèle (--only score / transfer /
decision en même temps), les 3 process se marchaient dessus et corrompaient
le checkpoint. Maintenant chaque famille a son propre fichier
(blackbox_score_<tag>.csv, blackbox_transfer_<tag>.csv,
blackbox_decision_<tag>.csv), et tu les fusionnes ensuite avec
merge_blackbox_results.py.

Usage (parallèle, recommandé) :
  python blackbox_multirun.py --dataset swat --eps 0.1 --only score &
  python blackbox_multirun.py --dataset swat --eps 0.1 --only transfer &
  python blackbox_multirun.py --dataset swat --eps 0.1 --only decision &
  wait
  python merge_blackbox_results.py --dataset swat --eps 0.1

Usage (tout en un, comme avant) :
  python blackbox_multirun.py --dataset swat --eps 0.1

Sorties :
  ~/<dataset>/results/blackbox_<family>_<dataset>_eps<eps>.csv
  ~/<dataset>/results/blackbox_<family>_<dataset>_eps<eps>.json
  ~/<dataset>/results/blackbox_<family>_<dataset>_eps<eps>_tmp.csv   (checkpoint)
  (family = "score" | "transfer" | "decision" | "multirun" si --only absent)
"""
...
# ✅ FIX : nom de fichier spécifique à la famille lancée, pour permettre
# le lancement en parallèle sans collision entre process.
FAMILY_TAG = args.only if args.only else "multirun"

OUT_CSV  = RESULTS_DIR / f"blackbox_{FAMILY_TAG}_{TAG}.csv"
TMP_CSV  = RESULTS_DIR / f"blackbox_{FAMILY_TAG}_{TAG}_tmp.csv"
OUT_JSON = RESULTS_DIR / f"blackbox_{FAMILY_TAG}_{TAG}.json"
...
N_SCORE_PER_SEED    = 2 * N_VICTIMS  if RUN_SCORE    else 0
N_TRANSFER_PER_SEED = (3 * 2 * N_VICTIMS + N_VICTIMS) if RUN_TRANSFER else 0
N_DECISION_PER_SEED = 2 * N_VICTIMS  if RUN_DECISION else 0
...
print(f"  Famille  : {FAMILY_TAG}")
print(f"  Familles activées : Score={RUN_SCORE} | Transfer={RUN_TRANSFER} | Decision={RUN_DECISION}")
...
print(f"  Sorties  : {RESULTS_DIR}  (fichiers taggés '{FAMILY_TAG}')")
print(f"{'═'*65}")
...
def run():
    ...
    if TMP_CSV.exists():
        ...
        _task_counter["skipped"] = len(all_results)
